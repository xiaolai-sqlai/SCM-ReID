...
if __name__ == '__main__':
    ...
    data_dir = osp.join(opt.dataset_root, "2018_Occluded_REID")
    os.environ["CUDA_VISIBLE_DEVICES"] = opt.gpus

    test_set = []
    test_label = []
    test_cam = []
    for label in sorted(os.listdir(osp.join(data_dir, 'whole_body_images'))):
        for name in sorted(os.listdir(osp.join(data_dir, 'whole_body_images', label))):
            if name.endswith(".tif"):
                test_set.append((osp.join(data_dir, 'whole_body_images', label, name), int(label)))
                test_label.append(int(label))
                # test_cam.append(int(label))
    ...
    query_label = []
    query_cam = []
    for label in sorted(os.listdir(osp.join(data_dir, 'occluded_body_images'))):
        for name in sorted(os.listdir(osp.join(data_dir, 'occluded_body_images', label))):
            if name.endswith(".tif"):
                query_set.append((osp.join(data_dir, 'occluded_body_images', label, name), int(label)))
                query_label.append(int(label))
                # query_cam.append(int(label) + len(test_cam))
    ...
    if opt.net == "regnet_y_1_6gf":
        opt.feat_num = 1024
    elif opt.net == "regnet_y_3_2gf":
        opt.feat_num = 1024
    elif opt.net == "regnet_y_8gf":
        opt.feat_num = 1024

    net = DBN(num_classes=751, num_parts=[1,opt.num_part], net=opt.net)
